...
JUTTexture::JUTTexture(int sizeX, int sizeY, GXTexFmt textureFormat)
{
    ...
    img->mTransparency = JUTTransparency::UNK0;
    img->mSizeX = sizeX;
    img->mSizeY = sizeY;
    img->mWrapS = GX_CLAMP;
    img->mWrapT = GX_CLAMP;
...
void JUTTexture::storeTIMG(const ResTIMG *img, JUTPalette *palette, GXTlut tlut)
{
    if (img)
    {
        ...
        if (palette)
        {
            ...
            {
                palette->storeTLUT(tlut, (GXTlutFmt)palette->getFormat(), palette->getTransparency(), palette->getNumColors(),
                                   palette->getColorTable());
            }
        }
