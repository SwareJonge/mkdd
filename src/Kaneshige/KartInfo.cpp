...
void KartInfo::setDriver(int driverNo, ECharID charID, KartGamePad * gamePad) {
    ...
    if(!assertFlag) {
        JUTAssertion::showAssert_f(JUTAssertion::getSDevice(), __FILE__, 632, "range over: %d <= driverNo=%d < %d", 0, driverNo, 2);
        OSPanic(__FILE__, 632, "Halt");
    }
...
void KartInfo::KartCharacter::setCharDB(const SCharDB  * sCharDB) {
    charDB = sCharDB;
}
